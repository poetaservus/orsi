...
class FilesystemCopyArguments(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True)
    source_path: str = Field(min_length=1, max_length=32767,
                            description="Exact absolute local-drive path of the regular file to copy.")
...
class FilesystemCopyCapability(Capability[FilesystemCopyArguments]):
    name = "filesystem.copy"
    description = "Copy one bounded regular file to an exact destination after approval."
    arguments_model = FilesystemCopyArguments
    ...
    def __init__(self, policy: HostWritePolicy):
        if not isinstance(policy, HostWritePolicy):
            raise TypeError("File copying requires a separate HostWritePolicy.")
        self.policy = policy
    ...
    def approval_preview(self, arguments, context) -> str:
        ...
        return "\n".join((
            ...
            "Source content will not be displayed in this preview.",
        ))

    def execute(self, arguments: FilesystemCopyArguments, context: CapabilityContext) -> dict:
        source, destination = self._paths(arguments)
        _ensure_distinct(source, destination)
        if (context.authorized_resource is None or destination != Path(context.authorized_resource)
            ...
            raise CapabilityExecutionError(CapabilityErrorCode.PERMISSION_DENIED,
                                           "File copying requires exact runtime authorization.")
        try:
            source_snapshot = _source_snapshot(source, context.cancellation)
            with pinned_parent(destination, context.cancellation) as (_, parent_identity):
                destination_state = _destination_state(destination)
                ...
                context.cancellation.raise_if_cancelled()
                identity = atomic_copy_file(source, destination, source_snapshot["sha256"],
                                            arguments.on_collision, context.cancellation)
                context.cancellation.raise_if_cancelled()
                if _file_sha256(destination, context.cancellation) != source_snapshot["sha256"]:
                    raise RuntimeError("The copied file content could not be verified.")
                return {"source_path": str(source), "destination_path": str(destination),
                        "copied": True, "operation": ("replaced" if destination_state != "missing" else "created"),
                        "collision_policy": arguments.on_collision,
                        "bytes_copied": source_snapshot["size_bytes"],
                        "sha256": source_snapshot["sha256"], "identity": identity}
        except OSError as exc:
            _raise_path_error(exc, source=source, destination=destination)

    def _paths(self, arguments: FilesystemCopyArguments) -> tuple[Path, Path]:
        source = self.policy.resolve_copy_source(arguments.source_path)
        destination = self.policy.resolve_copy_destination(arguments.destination_path)
        return source, destination


def atomic_copy_file(source: Path, destination: Path, expected_sha256: str,
                     on_collision: str, cancellation) -> str:
    ...
    try:
            ...
            while True:
                ...
                if total > _MAX_COPY_BYTES:
                    raise CapabilityExecutionError(CapabilityErrorCode.OUTPUT_LIMITED,
                        "File copying is limited to 16 MiB in this checkpoint.")
                digest.update(chunk)
                dst.write(chunk)
            dst.flush()
            os.fsync(dst.fileno())
        ...
        try:
            ...
            raise RuntimeError("The copied file could not be verified.") from exc
    except BaseException:
        if not placed and os.path.lexists(temp_path):
            try:
                os.unlink(temp_path)
            except OSError as exc:
                raise RuntimeError("The temporary copy file could not be removed safely.") from exc
        raise
...
def _temporary_path(path: Path) -> Path:
    for _ in range(8):
        candidate = path.with_name(f".orsi-copy-{uuid4().hex}.tmp")
        if not os.path.lexists(candidate):
            return candidate
    raise FileExistsError("A unique temporary copy file name could not be reserved.")
